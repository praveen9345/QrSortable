namespace QrSortable.Components.CoreFeatures.CodeGenerator
{
    ...
    {
        // ── A4 Label sheet physical dimensions (mm) ──────────────────────────
        ...
        // ── A5 Label sheet physical dimensions (mm) ──────────────────────────
        private const double A5PageWidthMm = 148.0;
        private const double A5PageHeightMm = 210.0;
        private const double A5CellWidthMm = 74.0;   // 2 × 74 = 148 → no side margin
        private const double A5CellHeightMm = 66.3;   // (210 - 10.1 top margin) / 3 rows
        private const double A5TopMarginMm = 10.1;

        // ── Shared inner layout (mm) ─────────────────────────────────────────
        ...
        // ────────────────────────────────────────────────────────────────────
        //  QR CODE PDF — A4 (12 codes per page, 3 cols × 4 rows)
        // ────────────────────────────────────────────────────────────────────
        public async Task<byte[]> GenerateQrPdfA4Async(List<ImageSource> qrcodes)
        {
        ...
        // ────────────────────────────────────────────────────────────────────
        //  BARCODE PDF — A4 (12 codes per page, 3 cols × 4 rows)
        // ────────────────────────────────────────────────────────────────────
        public async Task<byte[]> GenerateBarcodePdfA4Async(List<ImageSource> barcodes)
        {
            return await Task.Run(() =>
            {
                using var document = new PdfDocument();
        ...
        // ────────────────────────────────────────────────────────────────────
        //  QR CODE PDF — A5 (6 codes per page, 2 cols × 3 rows)
        // ────────────────────────────────────────────────────────────────────
        public async Task<byte[]> GenerateQrPdfA5Async(List<ImageSource> qrcodes)
        {
            return await Task.Run(() =>
            {
                using var document = new PdfDocument();
                ...
                double cellW = Mm(A5CellWidthMm);
                double cellH = Mm(A5CellHeightMm);
                double topM = Mm(A5TopMarginMm);
                double hPad = Mm(HorizPadMm);
                double vPad = Mm(VertPadMm);
                double labelH = Mm(LabelHeightMm);
                double labelGap = Mm(LabelGapMm);
                ...
                double maxW = cellW - 2 * hPad;
                double maxH = cellH - vPad - labelH - labelGap - vPad;
                double qrSize = Math.Min(maxW, maxH);
                ...
                {
                    var page = document.AddPage();
                    page.Width = Mm(A5PageWidthMm);
                    page.Height = Mm(A5PageHeightMm);
                    using var gfx = XGraphics.FromPdfPage(page);
        ...
        // ────────────────────────────────────────────────────────────────────
        //  BARCODE PDF — A5 (6 codes per page, 2 cols × 3 rows)
        // ────────────────────────────────────────────────────────────────────
        public async Task<byte[]> GenerateBarcodePdfA5Async(List<ImageSource> barcodes)
        {
            return await Task.Run(() =>
            {
                using var document = new PdfDocument();
